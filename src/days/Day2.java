 ...
 * On the radio, an Elf is already explaining how to handle the situation: "Don't worry, that's perfectly norma--" The ship
 ...
 * the positions of the two inputs (9 and 10), and the position of the output (3). To handle this opcode, you first need to
 * get the values at the input positions: position 9 contains 30, and position 10 contains 40. Add these numbers together to
 * get 70. Then, store this value at the output position; here, the output position (3) is at position 3, so it overwrites itself.
 ...
 * adding. The inputs are at positions 3 and 11; these positions contain 70 and 50 respectively. Multiplying these produces
 ...
 *
 * "With terminology out of the way, we're ready to proceed. To complete the gravity assist, you need to determine what pair
 * of inputs produces the output 19690720."
 *
 * The inputs should still be provided to the program by replacing the values at addresses 1 and 2, just like before. In this
 * program, the value placed in address 1 is called the noun, and the value placed in address 2 is called the verb. Each of
...
public class Day2 {
    ...
    public static void main(String[] args) {
        String[] rawInput = InputReader.getInput(2).split(",");
        int[] codes = Util.convertToInt(rawInput);

        System.out.println("Part 1: " + part1(codes));
        System.out.println("Part 2: " + part2(codes));
    }

    private static int part1(int[] codes) {
        int[] copy = Arrays.copyOf(codes, codes.length);
        copy[1] = 12;
        copy[2] = 2;

        return intComputer(copy);
    }

    private static int part2(int[] codes) {
        for(int i = 0; i < 100; i++) {
            for(int j = 0; j < 100; j++) {
                int[] copy = Arrays.copyOf(codes, codes.length);
                copy[1] = i;
    ...
    private static int intComputer(int[] codes) {
