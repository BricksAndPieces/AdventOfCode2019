 ...
 * position in space, safely align his warp drive, and return to Earth in time to save Christmas, he needs you to bring him measurements
 * from fifty stars.
 *
 ...
 * - The Fuel Counter-Upper needs to know the total fuel requirement. To find it, individually calculate the fuel needed for the mass
 ...
 * requires fuel, and that fuel requires fuel, and so on. Any mass that would require negative fuel should instead be treated as if
 * it requires zero fuel; the remaining mass, if any, is instead handled by wishing really hard, which has no mass and is outside
 ...
 *
 * - A module of mass 14 requires 2 fuel. This fuel requires no further fuel (2 divided by 3 and rounded down is 0, which would
 *   call for a negative fuel), so the total fuel required is still just 2.
 * - At first, a module of mass 1969 requires 654 fuel. Then, this fuel requires 216 more fuel (654 / 3 - 2). 216 then requires
 *   70 more fuel, which requires 21 fuel, which requires 5 fuel, which requires no further fuel. So, the total fuel required for
...
public class Day1 {
    ...
    public static void main(String[] args) {
        String[] rawInput = InputReader.getInput(1).split("\n");
        int[] masses = Util.convertToInt(rawInput);

        System.out.println("Part 1: " + part1(masses));
        System.out.println("Part 2: " + part2(masses));
    }

    private static int part1(int[] masses) {
        int totalFuel = 0;
        for(int mass : masses)
    ...
    private static int part2(int[] masses) {
        int totalFuel = 0;
        for(int mass : masses) {
